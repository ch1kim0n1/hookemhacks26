...
- [ ] `bash scripts/e2e-critique-check.sh` returns **ALL CHECKS PASSED** (covers the seven items from the 2026-04-18 technical critique).
- [ ] **If pitching with real proofs:** used `bash scripts/demo-production-up.sh` instead of plain `docker compose up` so `RISC0_DEV_MODE=0` and the three demo scenarios are pre-warmed in the proof cache.
