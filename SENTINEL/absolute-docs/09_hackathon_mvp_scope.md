...
- `learning-loop`: ZK prover wired to `/prove/learning`; per-generation stats accumulated and sent to real guest. **Shipped (beyond original scope).**
- `preemptive-strike`: Layer 6 mempool-watch engine. **Shipped (beyond original scope).**

### 3. ZK Circuits

- **PolicyCompliance** — LIVE. Real proof generation path, cache fallback.
- **CounterfactualCorrectness** — structured-claim guest (sum-of-deltas + SHA-256 Merkle consistency) with Hybrid Approach A forkBlockHash grounding. Journal = 160 bytes / 5 public inputs.
- **LearningLoopCorrectness** — **Shipped.** Proves aggregate win-rate floor across committed per-generation attack/defend counts.
...
**Cut:** reentrancy scenario, governance attack scenario.
**Bonus Scenario C — Pre-emptive Strike:** `preemptive-strike` service watches mempool for the attacker contract + `attack(address,uint256)` selector and fires `PauseController` before the tx lands. **Shipped.**

## Hard Scope — Out

These DO NOT SHIP for the hackathon. If you find yourself working on one, stop.

- ~~Layer 6 (Pre-emptive Strike Engine)~~ — **Shipped.** `services/preemptive-strike`.
- ~~Real Red/Blue adversarial training~~ — **ZK wiring shipped.** Learning-loop calls `/prove/learning`; visualization still pre-recorded for demo.
